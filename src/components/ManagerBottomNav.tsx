  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200/50 flex items-center justify-around p-2 pb-safe z-[1000] shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
      <Link 
        ...
        className={`flex flex-col items-center p-2 transition-colors ${path.includes('/manager/consumer') ? 'text-blue-600' : 'text-slate-500 hover:text-blue-600'}`}
      >
      ...
      <Link 
        to="/manager/agents" 
        className={`flex flex-col items-center p-2 transition-colors ${path.includes('/manager/agent') ? 'text-blue-600' : 'text-slate-500 hover:text-blue-600'}`}
      >
